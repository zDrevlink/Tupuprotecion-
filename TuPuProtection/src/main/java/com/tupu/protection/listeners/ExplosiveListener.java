...
public class ExplosiveListener implements Listener {
    ...
    public void onPlaceCustomTnt(PlayerInteractEvent event) {
        ...
        // Bukkit hace "saltar" ligeramente a la TNT recién generada (velocidad inicial
        // por defecto); la anulamos explícitamente para que la pegajosa quede
        // realmente quieta y no salga volando hacia arriba.
        ...
        if (sticky) {
            // Refuerzo por si alguna otra física (explosiones cercanas, empujones)
            // le diera velocidad: la mantenemos en cero mientras exista.
            new BukkitRunnable() {
                @Override
                public void run() {
                    ...
                    if (!tnt.getVelocity().equals(new Vector(0, 0, 0))) {
                        tnt.setVelocity(new Vector(0, 0, 0));
                    }
                }
